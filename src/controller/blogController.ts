...
export const addBlog = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!blogHeading || !postedDate || !writtenBy || !htmlContent) {
      ...
      const query = "INSERT INTO blogs (blogHeading, postedDate, writtenBy, htmlContent) VALUES (?)";
...
export const getAllBlogs = (req: Request, res: Response, next: NextFunction) => {
  try {
    const query = "SELECT blog_id, blogHeading, postedDate, writtenBy, htmlContent FROM blogs"
    db.getConnection(function (err, connection) {
      if (err) {
        return res.status(400).json({
          type: false,
        ...
        connection.query(query, (err: any, data: any) => {
          if (err) {
            ...
            const blogsData = data.map((blog: any) => {
              const convertedDate = new Date(blog.postedDate).toLocaleDateString();
              return {
...
export const getSingleBlog = (req: Request, res: Response, next: NextFunction) => {
  try {
    const blogId = req.params.id;
    const query = "SELECT blog_id, blogHeading, postedDate, writtenBy, htmlContent FROM blogs WHERE blog_id=?";
    db.getConnection(function (err, connection) {
      if (err) {
        return res.status(400).json({
          type: false,
        ...
        connection.query(query, [blogId], (err: any, data: any) => {
          if (err) {
            ...
            const convertedDate = new Date(data[0].postedDate).toLocaleDateString();
            const blogData = {
